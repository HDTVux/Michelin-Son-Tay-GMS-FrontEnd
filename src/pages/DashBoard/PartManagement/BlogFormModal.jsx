...
export default function BlogFormModal({ item, mode = 'create', onClose, onSaved }) {
  ...
  const draftStorageKey = useMemo(() => {
    ...
    return `gms_blog_form_draft_v1:${mode}:${itemId}`;
  ...
  const loadWorkCategoryOptions = useCallback(async () => {
    ...
    try {
      ...
      const normalized = normalizeWorkCategoryList(payload)
        .filter((entry) => entry.categoryType === normalizedItemType)
        .sort((a, b) => a.workCategoryId - b.workCategoryId);
      setWorkCategories(normalized);
      // Auto-select the first category by current item type on initial load
      if (itemCategoryId === `${normalizedItemType}_AUTO` && normalized.length > 0) {
        setItemCategoryId(String(normalized[0].workCategoryId));
      }
    } catch {
      // keep existing state
    } finally {
  ...
  }, [itemCategoryId, normalizedItemType]);
  ...
  const buildServiceFormData = useCallback(() => {
    ...
    return formData;
  }, [buildDescriptionHtml, existingMedia, introText, isActive, itemName, mediaFiles, price, priceMode, thumbnailFile, thumbnailPreview, warrantyMonths]);
  ...
  const handleCreateWorkCategory = useCallback(async () => {
    ...
    if (!categoryCode) {
      notify('Vui lòng nhập mã category.', 'error');
      return;
    }
    if (!categoryName) {
      notify('Vui lòng nhập tên category.', 'error');
      return;
    }
    try {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const token = localStorage.getItem('authToken') || localStorage.getItem('staffToken');
    if (!token) {
      notify('Vui long dang nhap.', 'error');
      return;
    }
    try {
      ...
      <div className={styles['modal-box']}>
        ...
        <div className={styles['modal-body']}>
          ...
          <div className={styles['section-label']}>
            {normalizedItemType === 'SERVICE' ? 'Thông tin dịch vụ' : 'Thông tin phụ tùng'}
          </div>
          ...
          <div className={styles.field}>
            ...
            <div className={styles['price-mode-row']}>
              <label className={styles['price-choice']}>
                <input type="radio" name="item-type" value={normalizedItemType} checked readOnly />
                {itemTypeLabel}
              </label>
            </div>
          </div>
          ...
          <div className={styles['field-row']}>
            <div className={styles.field}>
              <label htmlFor="item-sku">{itemCodeLabel} <span className={styles.required}>*</span></label>
          ...
          <div className={styles['field-row']}>
            ...
            <div className={styles.field}>
              <label htmlFor="item-price">{itemPriceLabel}</label>
              {priceMode === 'fixed' ? (
                <>
                  <input
                    id="item-price"
                ...
                <div className={styles['readonly-value']}>Giá: Liên hệ</div>
              )}
            </div>
          </div>
          ...
          <div className={styles.field}>
            ...
            />
            {errors.introText && <span className={styles['field-error']}>{errors.introText}</span>}
          </div>
          ...
          <div className={styles.field}>
            ...
            />
            {errors.detailHtml && <span className={styles['field-error']}>{errors.detailHtml}</span>}
          ...
          {isCreateNew && (
            <>
              <div className={styles['field-row']}>
                ...
                <div className={styles.field}>
                  <label htmlFor="product-line-id">Product Line ID</label>
                  <input
                    id="product-line-id"
                    type="number"
                    min="0"
              ...
              <div className={styles.field}>
                ...
                {categoryInputMode === 'select' ? (
                  <select value={itemCategoryId} onChange={(e) => setItemCategoryId(e.target.value)} disabled={isSubmitting || isLoadingWorkCategories}>
              ...
              <div className={styles['field-row']}>
                <div className={styles.field}>
                  <label htmlFor="new-work-category-code">Tạo category mới - Mã</label>
                  <input
                    id="new-work-category-code"
                    type="text"
                ...
                <div className={styles.field}>
                  <label htmlFor="new-work-category-name">Tên category</label>
                  <div className={styles['field']} style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
                    <input
                      id="new-work-category-name"
                      type="text"
                      ...
                      style={{ flex: 1 }}
                    />
                    <button type="button" className={styles['auto-gen-button']} onClick={handleCreateWorkCategory} disabled={isSubmitting || isCreatingWorkCategory}>
          ...
          <div className={styles['field-row']}>
            <div className={styles.field}>
              <label htmlFor="item-unit">Đơn vị <span className={styles.required}>*</span></label>
              <input
                id="item-unit"
                type="text"
                value={unit}
